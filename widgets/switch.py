...
class AnimatedSwitch(ctk.CTkFrame):
    def __init__(self, parent, width=80, height=30, fg_color="lime", handle_color="white", command=None, initial_state=False):
        ...
        self.width = width
        self.height = height
        self.bg_color = "#808080"  # Background color based on theme
        ...
        self.canvas = tk.Canvas(self, width=self.width, height=self.height, bd=0, highlightthickness=0)
        self.canvas.pack()
    ...
    def get_theme_color(self):
        """Get the color of the parent frame to match the background dynamically."""
        try:
            # Check if the parent is a CTk widget and get its foreground color, which changes with the theme
            if isinstance(self.master, ctk.CTkBaseClass):
                colors = self.master.cget("fg_color")
                # Handle both dark and light mode colors if it's a list
                if isinstance(colors, list) and len(colors) == 2:
                    appearance_mode = ctk.get_appearance_mode()
                    return colors[0] if appearance_mode == "Light" else colors[1]
                elif isinstance(colors, str):
                    return colors
            # Fallback to a default color if color retrieval fails
            return "#808080"  # Grey as a fallback if no color can be retrieved
        except:
            return "#808080"
    ...
    def create_pill_background(self, color=None):
        ...
        # Draw the rounded rectangle with color fill and a black outline
    ...
    def get_parent_color(self):
        """Get the background color of the parent frame dynamically."""
        try:
            # Check if the parent is a CTk widget and get its foreground color, adapting to light or dark mode
            if isinstance(self.master, ctk.CTkBaseClass):
                colors = self.master.cget("fg_color")
                if isinstance(colors, list) and len(colors) == 2:
                    appearance_mode = ctk.get_appearance_mode()
                    ...
                    return colors
            # Fallback color if the parent’s color is not accessible
            return "#808080"
        except:
            return "#808080"
    ...
    def update_theme(self):
        ...
        parent_color = self.get_parent_color()
        # Update the canvas background directly
        self.canvas.config(bg=parent_color)
        # Re-render the switch with the correct color for the OFF state or ON state
        self.create_pill_background(self.fg_color if self.on else parent_color)
        self.switch_bg = ImageTk.PhotoImage(self.pill_image)
        self.canvas.itemconfig(self.pill_item, image=self.switch_bg)
