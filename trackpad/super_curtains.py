...
def get_current_super_curtains_values():
    registry_path = r'SOFTWARE\Microsoft\Windows\CurrentVersion\PrecisionTouchPad'
    super_curtains_keys = ['SuperCurtainTop', 'SuperCurtainLeft', 'SuperCurtainRight', 'SuperCurtainBottom']
    super_curtains_values = {}
    
    try:
        reg_key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, registry_path, 0, winreg.KEY_READ)
        for key in super_curtains_keys:
            try:
                value, reg_type = winreg.QueryValueEx(reg_key, key)
                super_curtains_values[key] = value
            except FileNotFoundError:
                super_curtains_values[key] = None  # 값이 없는 경우 처리
        winreg.CloseKey(reg_key)
    except Exception as e:
        print(f"Error reading super curtains values: {e}")

    return super_curtains_values

# 레지스트리 값을 설정하는 함수
def set_super_curtains_values(super_curtains_values):
    registry_path = r'SOFTWARE\Microsoft\Windows\CurrentVersion\PrecisionTouchPad'
    
    try:
        reg_key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, registry_path, 0, winreg.KEY_SET_VALUE)
        for key, value in super_curtains_values.items():
            if value is not None:
                winreg.SetValueEx(reg_key, key, 0, winreg.REG_DWORD, value)
        winreg.CloseKey(reg_key)
        print("Super Curtains values set successfully.")
    except Exception as e:
        print(f"Error setting super curtains values: {e}")

# 'Super Curtains' 설정을 위한 서브 창 생성 함수
def create_super_curtains_window():
    sub_window = tk.Toplevel()
    sub_window.title("Super Curtains Settings")

    # Super Curtains 관련 레지스트리 값과 기본값, 범위 설정
    super_curtains_settings = [
        ("SuperCurtainTop", 0, 100, 50),
        ("SuperCurtainLeft", 0, 100, 50),
        ("SuperCurtainRight", 0, 100, 50),
        ("SuperCurtainBottom", 0, 100, 50)
    ]

    # 슬라이더와 상태 레이블을 생성하는 반복문
    for (key, min_value, max_value, default_value) in super_curtains_settings:
